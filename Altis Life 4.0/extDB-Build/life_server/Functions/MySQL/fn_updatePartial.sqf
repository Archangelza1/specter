...
switch(_mode) do {
	...
	case 2: {
		...
		switch(_side) do {
			...
			case east: {_query = format["UPDATE players SET bounty_licenses='%1' WHERE playerid='%2'",_value,_uid];};
		};
	};
	...
	case 3: {
		...
		switch(_side) do {
			...
			case east: {_query = format["UPDATE players SET bounty_gear='%1' WHERE playerid='%2'",_value,_uid];};
		};
	};
